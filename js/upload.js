...
// export let images = JSON.parse(localStorage.getItem("saveImage")) || [];

// function generateContent(){
//    setTimeout(() =>{
//       document.querySelector('.uplolad-container').style.display = 'block';
...
// //let testImg = ['Pictures/AIBOT.jpg', 'Pictures/PHO-p1.png', 'Pictures/PHO-p2.png'];
...
//    document.querySelector('.uploadNow')?.addEventListener('click', () => {
//       try{
//          if (saves.length > 0) {
//             images = [...images, ...saves];
//             localStorage.setItem("saveImage", JSON.stringify(images)); //Update the localstorage
...
//             resolve(event.target.result.map(imageRecord => imageRecord.data));
//          };
...
// // Step 1: Open IndexedDB and create an object store
...
// // Step 2: Save images to IndexedDB
// function saveImageToIndexedDB(db, image) {
//    return new Promise((resolve, reject) => {
...
// // Step 3: (Revised) Load images from IndexedDB using getImagesFromIndexedDB
// async function loadImagesFromIndexedDB(db) {
//    try {
...
// // Step 4: Generate content (unchanged)
// function generateContent(){
//   setTimeout(() =>{
...
//    images = await loadImagesFromIndexedDB(db);
